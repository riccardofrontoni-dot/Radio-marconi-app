...
export async function GET() {
  ...
  try {
    // 1. Chiamata live a Meta Graph API
    ...
    if (!data.followers_count && data.followers_count !== 0) {
      return NextResponse.json({ error: "Impossibile recuperare i follower", details: data }, { status: 400 });
    }

    const currentFollowers = data.followers_count;

    // 2. Inserimento del nuovo valore su Supabase
