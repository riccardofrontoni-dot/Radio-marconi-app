...
export default async function IMieiScriptSocialPage({
  searchParams,
}: {
  ...
  if (profile.reparto !== "social") {
    return (
      <div>
        <h2 style={{ fontSize: 22, marginBottom: 10 }}>I miei script social</h2>
        <p style={{ color: "var(--gray-text)", fontSize: 14 }}>Questa sezione è per il reparto Social.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6, flexWrap: "wrap", gap: 10 }}>
        <h2 style={{ fontSize: 22 }}>I miei script social</h2>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Link href={`/dashboard/social-script?mese=${mesePrec}`} style={navBtnStyle}>‹</Link>
          <span style={{ fontSize: 13.5, fontWeight: 600, minWidth: 110, textAlign: "center" }}>{MESI[mese]} {anno}</span>
          <Link href={`/dashboard/social-script?mese=${meseSucc}`} style={navBtnStyle}>›</Link>
        </div>
      </div>
      <p style={{ color: "var(--gray-text)", fontSize: 13, marginBottom: 24 }}>
        Le tue giornate di registrazione del mese — scrivi o modifica lo script da qui.
      </p>

      {eventi.length === 0 && (
        <p className="placeholder-note" style={{ marginTop: 0 }}>Nessuna giornata di registrazione assegnata a te questo mese.</p>
      )}
        ...
        {eventi.map((e) => {
          ...
          return (
            <div key={e.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "14px 16px", border: "1px solid var(--border)", borderRadius: 12 }}>
              ...
              <Link href={`/dashboard/social-script/${e.id}`} className="btn-primary" style={{ fontSize: 12, padding: "8px 14px", textDecoration: "none" }}>
