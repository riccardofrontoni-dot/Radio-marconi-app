...
import { inviaAvviso, creaFormat, eliminaFormat } from "@/lib/actions";
...
export default function MembriRepartoClient({
  ...
  function invia() {
    ...
    startTransition(async () => {
      ...
      mostraToast(`Messaggio inviato a ${membro?.full_name?.split(" ")[0] || "membro"}`);
      setTestoAvviso("");
      router.refresh();
    });
  }
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--gray-text)", fontSize: 13, marginBottom: 24 }}>
        Clicca un membro per mandargli un messaggio diretto.
      </p>
      ...
      <div className="section-label" style={{ marginTop: 0 }}>Membri</div>
        ...
        {membri.map((m) => (
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 11.5, color: "var(--gray-text)", marginTop: 2 }}>{m.email}</div>
            </div>
          </button>
        ))}
      </div>
        ...
        {membro && (
          <>
            ...
            <div style={{ marginTop: "auto" }}>
              <label style={labelStyle}>Manda un messaggio</label>
              <p style={{ fontSize: 11.5, color: "var(--gray-text)", marginTop: -2, marginBottom: 10 }}>
                Un messaggio diretto, diverso da una task — per assegnare task usa "Gestione task".
              </p>
              <textarea value={testoAvviso} onChange={(e) => setTestoAvviso(e.target.value)} placeholder="Un messaggio rapido..." style={{ ...inputStyle, minHeight: 90, resize: "vertical", marginBottom: 10 }} />
              <button onClick={invia} disabled={isPending || !testoAvviso.trim()} className="btn-primary" style={{ width: "100%", background: colore }}>
                Invia messaggio
              </button>
            </div>
          </>
